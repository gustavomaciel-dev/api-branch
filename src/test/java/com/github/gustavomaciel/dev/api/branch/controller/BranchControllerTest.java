...
public class BranchControllerTest {
  ...
  public void createBranchTest() throws Exception {
    Address address = Address.builder().street("mendoza").latitude(1234D).longitude(4321D).build();
    BranchDTO dto = createBranch();
    Branch savedBranch = Branch.builder().id(1L).address(address).build();

    BDDMockito.given(service.save(Mockito.any(Branch.class))).willReturn(savedBranch);
    String json = new ObjectMapper().writeValueAsString(dto);

    MockHttpServletRequestBuilder request = MockMvcRequestBuilders.post(BRANCH_API)
  ...
  public void getBranchTest() throws Exception {
    ...
    MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get(BRANCH_API.concat("/" + id))
        .accept(MediaType.APPLICATION_JSON);

    mvc.perform(request)
        .andExpect(status().isOk())
        .andExpect(jsonPath("branchId").value(1))
        .andExpect(jsonPath("street").value(createBranch().getStreet()))
  ...
  public void branchNotFoundTest() throws Exception {
    ...
    MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get(BRANCH_API.concat("/" + 1))
        .accept(MediaType.APPLICATION_JSON);
  ...
  public void findAllBranchTest() throws Exception {
    ...
    Branch branchRequest = Branch.builder().id(1L)
        .address(Address.builder().street("calle808").latitude(1235D).longitude(4322D).build()).build();
    ;

    Branch nearestBranch = Branch.builder().id(1L)
        .address(Address.builder().street("calle808").latitude(1234D).longitude(4321D).build()).build();
    ...
    BDDMockito.given(service.getAll()).willReturn(Arrays.asList(branch2, nearestBranch, branch3));

    String queryString = String.format("/nearest?latitude=%s&longitude=%s&page=0&size=0", branchRequest.getAddress().getLatitude(),
        branchRequest.getAddress().getLongitude());
    MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get(BRANCH_API.concat(queryString))
        .accept(MediaType.APPLICATION_JSON);

    mvc.perform(request)
        .andExpect(status().isOk())
        .andExpect(jsonPath("branchId").value(nearestBranch.getId()))
        .andExpect(jsonPath("street").value(nearestBranch.getAddress().getStreet()))
        .andExpect(jsonPath("latitude").value(nearestBranch.getAddress().getLatitude()))
        .andExpect(jsonPath("longitude").value(nearestBranch.getAddress().getLongitude()));
  }

  private BranchDTO createBranch() {
    BranchDTO dto = BranchDTO.builder().street("mendoza").latitude(1234D).longitude(4321D).build();
    return dto;
  }
